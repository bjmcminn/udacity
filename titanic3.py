...
def simple_heuristic():
    ...
    for passenger_index, passenger in df.iterrows():
        #print str(passenger['PassengerId']) + ' ' + str(passenger['Sex'])
       
        pID = passenger['PassengerId']
        theSex = passenger['Sex']
        theClass = passenger['Pclass']
        theAge = passenger['Age']

        blnFemaleExclude = (theClass == 3 and theAge > 30 and theAge < 50)
        
        if (theSex == 'female' and blnFemaleExclude == 0) or ((theClass == 1 or theClass == 2) and theAge < 18):
            survived = 1
        else:
            survived = 0

        predictions[pID] = survived
