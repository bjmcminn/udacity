...
def lineplot(hr_year_csv):
    ...
    hr_data = pandas.read_csv(hr_year_csv)
    gg = ggplot(hr_data, aes('yearID','HR')) + geom_point(color = 'red') + geom_line(color='red') + ggtitle('HRs by Year') + xlab('Year') + ylab('y-#HomeRuns')
    return gg


if __name__ == "__main__":

    filename = "baseball_data_raw/hr_year.csv"
    
    print lineplot(filename)
